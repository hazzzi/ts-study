...
## 5
- 민감정보: 세션 키, DB 접속을 위한 정보(host, user, password...)
- 하드 코딩
  - 설정 정보가 변경되면 코드를 수정해야 함, 배포도 다시해야함
- 설정파일 사용
  - SCM(soure code management) 배포시 주의
  - .gitignore에 추가, db-config-sample.js과 같은 예제 파일을 등록하고 사용법 명시
- command-line에서 arg 요구
  - 앱을 실행할 때마다 입력해야할 정보의 양이 많아진다.
  - script를 이용하여 해결한다면 비밀정보가 script내에 존재하는 문제
- 환경변수 사용